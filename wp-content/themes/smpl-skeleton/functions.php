...
if (! function_exists ( 'skeleton_scripts' )) {
	function skeleton_scripts() {
		...
		wp_enqueue_script ( 'superfish', get_template_directory_uri () . "/javascripts/superfish.js", array (
				'jquery' 
		), $version, true );
		wp_enqueue_script ( 'custom', get_template_directory_uri () . "/javascripts/custom.js", array (
				'jquery' 
		), $version, true );
...
if (! function_exists ( 'skeleton_customizer_styles' )) {
	function skeleton_customizer_styles() {
		...
		$body_query_args = array (
				'family' => skeleton_options ( 'body_font' ) . ':400,700' 
		);
		$heading_query_args = array (
				'family' => skeleton_options ( 'heading_font' ) . ':400,700' 
		);
...
if (! function_exists ( 'skeleton_display_thumbnail' )) {
	function skeleton_display_thumbnail() {
		...
		$image = get_the_post_thumbnail ( $id, $size, array (
				'class' => $align 
		) );
		echo $image;
	}
...
if (! function_exists ( 'skeleton_custom_pagenav' )) :
	function skeleton_custom_pagenav() {
		...
		if (function_exists ( 'wp_pagenavi' )) {
			...
			if (is_page ()) {
				wp_link_pages ( array (
						'before' => '<div class="page-link">' . __ ( 'Pages:', 'smpl' ),
						'after' => '</div>' 
				) );
			} elseif (is_single ()) {
				previous_post_link ( '<div class="nav-prev">%link</div>', __ ( 'Previous Post', 'smpl' ) );
...
function skeleton_customize_register($wp_customize) {
	...
	$wp_customize->add_setting ( 'skeleton_options[layout_info]', array (
			'sanitize_callback' => 'skeleton_sanitize_integer' 
	) );
...
function skeleton_customize_preview_js() {
	wp_enqueue_script ( 'skeleton_customizer', get_template_directory_uri () . '/javascripts/customizer.js', array (
			'jquery',
			'customize-preview' 
	), '20150805', true );
}
