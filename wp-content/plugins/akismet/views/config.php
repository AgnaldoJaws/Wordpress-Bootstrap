<div class="wrap">
	...
	<div class="have-key">
		...
		<?php if ( $stat_totals && isset( $stat_totals['all'] ) && (int) $stat_totals['all']->spam > 0 ) : ?>
			...
			<span style="float: right; margin: 10px 15px -5px 0px"> <a
				href="<?php echo esc_url( Akismet_Admin::get_page_url( 'stats' ) ); ?>"
			...
			<ul>
				<li>
					<h3><?php esc_html_e( 'Past six months' , 'akismet');?></h3> <span><?php echo number_format( $stat_totals['6-months']->spam );?></span>
						<?php esc_html_e( 'Spam blocked' , 'akismet');?>
					</li>
				<li>
					<h3><?php esc_html_e( 'All time' , 'akismet');?></h3> <span><?php echo number_format( $stat_totals['all']->spam );?></span>
						<?php esc_html_e( 'Spam blocked' , 'akismet');?>
					</li>
				<li>
					<h3><?php esc_html_e( 'Accuracy' , 'akismet');?></h3> <span><?php echo $stat_totals['all']->accuracy; ?>%</span>
						<?php
			
printf ( esc_html ( _n ( '%s missed spam, %s false positive', '%s missed spam, %s false positives', $stat_totals ['all']->false_positives, 'akismet' ) ), number_format ( $stat_totals ['all']->missed_spam ), number_format ( $stat_totals ['all']->false_positives ) );
			?>
					</li>
			</ul>
		...
		<?php if ( $akismet_user ):?>
			...
			<div class="postbox-container"
							...
							<div class="inside">
								<table cellspacing="0" class="akismet-settings">
									<tbody>
											...
											<td width="5%" />
											<td align="left"><span class="api-key"><input id="key"
													name="key" type="text" size="15" maxlength="12"
				...
				if (! function_exists ( 'wp_http_supports' )) {
					?><b><?php esc_html_e( 'Disabled.', 'akismet' ); ?></b> <?php
...
esc_html_e ( 'Your Web server cannot make SSL requests; contact your Web host and ask them to add support for SSL requests.', 'akismet' );
					...
					if ($ssl_disabled) {
						?><b><?php esc_html_e( 'Temporarily disabled.', 'akismet' ); ?></b> <?php
...
esc_html_e ( 'Akismet encountered a problem with a previous SSL request and disabled it temporarily. It will begin using SSL for requests again shortly.', 'akismet' );
						...
						?><b><?php esc_html_e( 'Enabled.', 'akismet' ); ?></b> <?php
...
esc_html_e ( 'All systems functional.', 'akismet' );
											...
											<td align="left">
												<p>
													<label for="akismet_show_user_comments_approved"
														...
														type="checkbox"
														<?php checked('1', get_option('akismet_show_user_comments_approved')); ?>> <?php esc_html_e('Show the number of approved comments beside each comment author', 'akismet'); ?></label>
												</p>
											</td>
										</tr>
										<tr>
											...
											<td align="left">
												<fieldset>
													...
													<p>
														<label for="akismet_strictness_1"><input type="radio"
															name="akismet_strictness" id="akismet_strictness_1"
															value="1"
															<?php checked('1', get_option('akismet_strictness')); ?> /> <?php esc_html_e('Silently discard the worst and most pervasive spam so I never see it.', 'akismet'); ?></label>
													</p>
													<p>
														<label for="akismet_strictness_0"><input type="radio"
															name="akismet_strictness" id="akismet_strictness_0"
															value="0"
															<?php checked('0', get_option('akismet_strictness')); ?> /> <?php esc_html_e('Always put spam in the Spam folder for review.', 'akismet'); ?></label>
													</p>
			...
			?>
							...
							<div id="major-publishing-actions">
									...
									<input type="hidden" name="action" value="enter-key"> <input
							...
							<table cellspacing="0">
								<tbody>
									<tr>
										...
										<td width="5%" />
										<td align="left"><span><?php echo $akismet_user->account_name; ?></span>
										</td>
									</tr>
									<tr>
			...
			?></span></td>
										...
										<td width="5%" />
										<td align="left"><span><?php echo date( 'F j, Y', $akismet_user->next_billing_date ); ?></span>
										</td>
									</tr>
										<?php endif; ?>
									</tbody>
