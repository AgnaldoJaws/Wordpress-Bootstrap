...
class Akismet {
	...
	private static function init_hooks() {
		...
		add_action ( 'akismet_scheduled_delete', array (
				'Akismet',
				'delete_old_comments' 
		) );
		add_action ( 'akismet_scheduled_delete', array (
		...
		add_action ( 'akismet_schedule_cron_recheck', array (
				'Akismet',
				'cron_recheck' 
		) );
		...
		add_action ( 'admin_head-edit-comments.php', array (
				'Akismet',
				'load_form_js' 
		) );
		add_action ( 'comment_form', array (
		...
		add_filter ( 'pre_comment_approved', array (
				'Akismet',
				'last_comment_status' 
		), 10, 2 );
	...
	public static function auto_check_comment($commentdata) {
		...
		$commentdata ['comment_as_submitted'] = array_intersect_key ( $comment, array (
				'blog' => '',
				'blog_charset' => '',
				'blog_lang' => '',
				'blog_ua' => '',
	...
	public static function set_last_comment($comment) {
		if (is_null ( $comment )) {
			...
			self::$last_comment = wp_filter_comment ( array_merge ( array (
	...
	public static function delete_old_comments() {
		 ...
		 *
		 * @param
		 *        	int The default number of days.
		 */
	...
	public static function transition_comment_status($new_status, $old_status, $comment) {
		...
		if ((isset ( $_POST ['status'] ) && in_array ( $_POST ['status'], array (
				'spam',
				'unspam' 
		) )) || (isset ( $_POST ['spam'] ) && ( int ) $_POST ['spam'] == 1) || (isset ( $_POST ['unspam'] ) && ( int ) $_POST ['unspam'] == 1) || (isset ( $_POST ['comment_status'] ) && in_array ( $_POST ['comment_status'], array (
				'spam',
				'unspam' 
		) )) || (isset ( $_GET ['action'] ) && in_array ( $_GET ['action'], array (
				'spam',
				'unspam' 
		) )) || (isset ( $_POST ['action'] ) && in_array ( $_POST ['action'], array (
				'editedcomment' 
		) ))) {
	 ...
	 *
	 * @param mixed $comment1
	 *        	A comment object or array.
	 * @param mixed $comment2
	 *        	A comment object or array.
	...
	private static function comments_match($comment1, $comment2) {
		...
		return (isset ( $comment1 ['comment_post_ID'], $comment2 ['comment_post_ID'] ) && intval ( $comment1 ['comment_post_ID'] ) == intval ( $comment2 ['comment_post_ID'] ) && $comment1 ['comment_author'] == $comment2 ['comment_author'] && $comment1 ['comment_author_email'] == $comment2 ['comment_author_email']);
	}
	 ...
	 *
	 * @param array $emails
	 *        	An array of email addresses that the moderation email will be sent to.
	 ...
	 * @param string $ip
	 *        	The specific IP address to hit.
	...
	public static function http_post($request, $path, $ip = null) {
		...
		if (! $ssl_disabled && function_exists ( 'wp_http_supports' ) && ($ssl = wp_http_supports ( array (
				'ssl' 
		) ))) {
